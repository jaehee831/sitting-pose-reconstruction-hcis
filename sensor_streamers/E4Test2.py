...
serverAddress = '127.0.0.1' # '172.27.182.18' # '127.0.0.8'
serverPort = 30000
bufferSize = 4096
...
def prepare_LSL_streaming():
    ...
    if bvp:
        infoBVP = pylsl.StreamInfo('bvp', 'BVP', 1, 64,'float32','E4_Bvp')
        global outletBVP
        outletBVP = pylsl.StreamOutlet(infoBVP)
    if gsr:
        infoGSR = pylsl.StreamInfo('gsr', 'GSR', 1, 4,'float32', 'E4_Gsr')
        global outletGSR
        outletGSR = pylsl.StreamOutlet(infoGSR)
    if tmp:
        infoTemp = pylsl.StreamInfo('tmp', 'Temp', 1, 4, 'float32', 'E4_Temperature')
        global outletTemp

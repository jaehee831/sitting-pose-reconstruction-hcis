...
from dataloader_v2 import PressureDataset
import torch
...
def run_epoch_v2(model, optimizer, loader, batch_size, cuda, cuda_idx, step=True):
    ...
    for i, (x, speed, motion_class, angle_class) in enumerate(loader):
        if cuda:
            x, speed, motion_class, angle_class = \
                x.cuda(cuda_idx), speed.cuda(cuda_idx), motion_class.cuda(cuda_idx), angle_class.cuda(cuda_idx)
        if len(motion_class.size()) > 1: motion_class = motion_class.squeeze()
        if len(angle_class.size()) > 1: angle_class = angle_class.squeeze()


        # print(i, len(loader))

        pred_motion = model(x.unsqueeze(dim=2))
...
def main(args):
    ...
    vaild_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True)
    ...
    for epoch in range(max_epoch):
        ...
        model.eval()
        valid_history = run_epoch_v2(model, optimizer, vaild_loader, batch_size, cuda, cuda_idx, step=False)

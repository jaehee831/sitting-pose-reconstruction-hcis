...
# This class used for single sensor
class Sensor:
...
# This class used for multi-sensor
class MultiSensors:
    def __init__(self, 
                ports = ['COM15'],
                normalize = False):
...
def main(
    ...
    while storage.frameCount < max_frame:
        ...
        joystickValue = joystick.get_axis(1) * -1
        if joystickValue > 0:
            joystickValue = 1
        elif joystickValue <= 0:
            joystickValue = -1
        ...
        #store data
        storage.addFrame(ts, {'pressure': total_image}, joystickValue)

        #verbosek
        print(f"FPS : {fps}, Frames : {storage.frameCount}, Storage : {foldername}/{storage.getName()}, class: {joystickValue}")
